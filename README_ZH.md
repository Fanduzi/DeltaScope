...
- **主键事实支持**：PostgreSQL `CREATE TABLE` 的内联、表级、命名和复合主键声明现在会填充 DeltaScope 标准化主键契约。已有的主键规则（`ddl.table.primary_key.bigint.require`、`ddl.table.primary_key.columns.max_count`）现在可以审计 PostgreSQL `CREATE TABLE` 语句。
...
- **Generated/identity 状态转换形态**——`ALTER TABLE ... ALTER COLUMN ... DROP EXPRESSION`、`SET GENERATED ALWAYS`、`SET GENERATED BY DEFAULT` 和 `DROP IDENTITY`——通过正常审核路径处理，并产生明确的 PostgreSQL-only 规则 findings（`ddl.alter.drop_expression.forbid`、`ddl.alter.set_generated.forbid`、`ddl.alter.drop_identity.forbid`）。
