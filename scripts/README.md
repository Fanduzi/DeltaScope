...
| verify_release_consistency.py | Validates release semantic consistency: release sequence that distinguishes unpublished failed-tag notes/history from current successfully published release surfaces, residual census arithmetic, SQL corpus metrics, the current supported rule-and-dialect fixture coverage label, PG ALTER TABLE rule count, required rule IDs across EN/ZH surfaces, no-overclaim wording, and no-leak wording |
