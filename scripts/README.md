...
| verify_release_consistency.py | Validates release semantic consistency: release sequence and every displayed landing release card's paired version, i18n keys, and EN/ZH release-note hrefs; residual census arithmetic; SQL corpus metrics; the current supported rule-and-dialect fixture coverage label; PG ALTER TABLE rule count; required rule IDs across EN/ZH surfaces; no-overclaim wording; and no-leak wording |
