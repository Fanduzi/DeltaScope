...
| extract.go | Converts parsed statements into first-pass domain `Statement` values with separate DDL constraint handling, typed index metadata, create-table shape flags, richer alter-table column/index/rename/option payloads, honest statement-local alter change facts, multi-column add normalization, and DML operation facts for offline governance |
| extract_test.go | Verifies representative DDL and DML extraction behavior, including create-like/create-as/partition flags plus enriched create-table facts and richer alter-table detail, including explicit statement-local change facts, multi-column add expansion, and non-index constraint handling |
