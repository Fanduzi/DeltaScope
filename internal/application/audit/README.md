...
| extract.go | Converts parsed statements into first-pass domain `Statement` values with separate DDL constraint handling, typed index metadata, create-table shape flags, richer DML operation facts, and column defaults/nullability/unsigned/auto-increment/current-timestamp extraction for offline DDL governance |
| extract_test.go | Verifies representative DDL and DML extraction behavior, including create-like/create-as/partition flags plus enriched create-table column and index facts |
