...
| extract.go | Converts parsed statements into first-pass domain `Statement` values with separate DDL constraint handling, typed index metadata, create-table shape flags, richer DML operation facts, and column defaults/nullability/current-timestamp extraction for offline DDL governance |
