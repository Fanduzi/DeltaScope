...
- `Diagnostic` carries structured evidence about unaudited or unsupported outcomes with five JSON fields: `classification` (stable category: `parser_error` or `unsupported_statement`), `reason` (safe human-readable explanation), `action_hint` (generic next step), `audited` (always `false` for diagnostic outcomes), and `dialect` (selected dialect when available). Two optional fields — `guidance_code` (stable machine-readable unsupported-boundary category) and `evidence_ref` (GitHub documentation URL) — appear only when classification is `parser_error` and the statement form matches a known unsupported boundary. The no-leak contract prohibits raw SQL text, parser `near ...` fragments, routine bodies, and all other forbidden payloads. `action_hint` must stay generic — it must not suggest object names inferred from failed parse text.
