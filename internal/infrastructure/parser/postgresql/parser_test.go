...
func TestExtractAlterTableDropGeneratedExpressionCurrentFacts(t *testing.T) {
	...
	// A. Confirm currently unsupported.
	statement := extractPostgreSQLStatement(t, sql)
	if statement.Unsupported == nil {
		t.Fatal("expected unsupported detail for DROP EXPRESSION")
	}
	...
	// B. Current visible normalized shape: the extractor returns empty Alter
	// immediately from alterFromCmd when it sees AT_DropExpression, so the
	// normalized output has no DDL payload. Only the AST-level facts are visible:
	// column name = cmd.GetName(), no def payload.
	//
	// This confirms the current extractor does NOT expose any transition-specific
	// facts in the normalized output for DROP EXPRESSION. The column name and
	// transition intent are available in the raw AST but are discarded before
	// reaching spec.Alter.

	if statement.Kind != spec.KindUnknown {
		t.Fatalf("expected kind unknown (unsupported), got %q", statement.Kind)
	}
	if statement.DDL != nil {
		t.Fatalf("expected nil DDL for unsupported statement, got operation=%q", statement.DDL.Operation)
	}

	// Verify raw AST still carries the facts we need.
...
func TestExtractAlterTableSetGeneratedAlwaysCurrentFacts(t *testing.T) {
	...
	// B. Current visible normalized shape: no DDL payload, all facts discarded.
	if statement.Kind != spec.KindUnknown {
		t.Fatalf("expected kind unknown (unsupported), got %q", statement.Kind)
	}
	if statement.DDL != nil {
	...
	// Verify raw AST carries transition facts.
	ast := parseAlterTableStmtAST(t, sql)
	cmds := ast.GetCmds()
	cmd := cmds[0].GetAlterTableCmd()
	if cmd.GetSubtype() != pg_query.AlterTableType_AT_SetIdentity {
...
func TestExtractAlterTableSetGeneratedByDefaultCurrentFacts(t *testing.T) {
	...
	statement := extractPostgreSQLStatement(t, sql)
	if statement.Unsupported == nil {
		t.Fatal("expected unsupported detail for SET GENERATED BY DEFAULT")
	}
	if statement.Unsupported.Feature != "generated_as_identity" {
		t.Fatalf("expected unsupported feature generated_as_identity, got %q", statement.Unsupported.Feature)
	}

	// B. Current visible normalized shape: no DDL payload.
	if statement.Kind != spec.KindUnknown {
		t.Fatalf("expected kind unknown (unsupported), got %q", statement.Kind)
	}
	if statement.DDL != nil {
	...
	// Verify raw AST carries transition facts with BY DEFAULT (100 = 'd').
	ast := parseAlterTableStmtAST(t, sql)
	cmds := ast.GetCmds()
	cmd := cmds[0].GetAlterTableCmd()
	if cmd.GetSubtype() != pg_query.AlterTableType_AT_SetIdentity {
...
func TestExtractAlterTableDropIdentityCurrentFacts(t *testing.T) {
	...
	// Verify raw AST carries the facts.
	ast := parseAlterTableStmtAST(t, sql)
	cmds := ast.GetCmds()
	cmd := cmds[0].GetAlterTableCmd()
	if cmd.GetSubtype() != pg_query.AlterTableType_AT_DropIdentity {
