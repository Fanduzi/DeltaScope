...
- The extractor normalizes PostgreSQL enum type lifecycle DDL (`CREATE TYPE ... AS ENUM`, `ALTER TYPE ... ADD VALUE`, `DROP TYPE`) into `spec.DDL` with `ObjectName` / `ObjectType` and coarse options (`type_kind`, `labels`, `action`, `value`, `if_not_exists`, `placement`, `neighbor`, `if_exists`, `cascade`). Composite type creation (`CREATE TYPE ... AS (...)`) returns explicit unsupported details with feature `create_type_composite`. DeltaScope does not inspect live type dependencies or enum usage.
