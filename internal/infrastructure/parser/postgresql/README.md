...
- The extractor normalizes PostgreSQL enum type lifecycle DDL (`CREATE TYPE ... AS ENUM`, `ALTER TYPE ... ADD VALUE`, `DROP TYPE`) into `spec.DDL` with `ObjectName` / `ObjectType` and coarse options (`type_kind`, `labels`, `action`, `value`, `if_not_exists`, `placement`, `neighbor`, `if_exists`, `cascade`). Composite type creation (`CREATE TYPE ... AS (...)`) and domain creation (`CREATE DOMAIN ...`) return explicit unsupported details with features `create_type_composite` and `create_domain`. DeltaScope does not inspect live type dependencies or enum usage.
