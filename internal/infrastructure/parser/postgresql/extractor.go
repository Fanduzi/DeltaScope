...
func (e pgExtractor) Extract(dialect spec.Dialect, rawSQL string) (spec.Statement, error) {
	...
	case *pg_query.Node_CreateEventTrigStmt:
		return extractCreateEventTrigStmt(statement, node.CreateEventTrigStmt), nil
	...
	case *pg_query.Node_CreateTransformStmt:
		return extractCreateTransformStmt(statement, node.CreateTransformStmt), nil
	case *pg_query.Node_CreateAmStmt:
		return extractCreateAmStmt(statement, node.CreateAmStmt), nil
	...
	case *pg_query.Node_AlterStatsStmt:
		return extractAlterStatsStmt(statement, node.AlterStatsStmt), nil
