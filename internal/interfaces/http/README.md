...
| query_access_e2e_mixed_literal_test.go | Docker-backed HTTP smoke for admitted and fail-closed MySQL 8.4 and TiDB 8.5 routes, including unqualified schema-only resolution through named connections, with response and access-log scans that keep registered DSN credential markers out of admitted paths, plus HTTP default/offline and bounded credential-failure no-leak coverage |
