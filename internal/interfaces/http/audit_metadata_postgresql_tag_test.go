...
func TestExecuteAuditRequestPostgreSQLAlterTableGeneratedFollowUpBoundariesReturnUnsupported(t *testing.T) {
	cases := map[string]struct {
		sql     string
		feature string
	}{
...
// TestExecuteAuditRequestPostgreSQLGeneratedIdentityUnsupportedMetadata proves that
// HTTP surface captures unsupported metadata for generated/identity columns via the
// result-capture closure pattern. Each case asserts column, generated_when, and
// (for identity) is_identity / identity_options flow through to the captured result.
func TestExecuteAuditRequestPostgreSQLGeneratedIdentityUnsupportedMetadata(t *testing.T) {
	cases := map[string]struct {
		sql              string
		feature          string
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if captured.Unsupported[0].Metadata == nil {
				t.Fatal("expected unsupported metadata to be populated")
			}

			for _, key := range tc.wantMetadataKeys {
				if _, ok := captured.Unsupported[0].Metadata[key]; !ok {
					t.Fatalf("expected metadata key %q, got %+v", key, captured.Unsupported[0].Metadata)
				}
			}
			for key, wantVal := range tc.wantMetadata {
				actVal, ok := captured.Unsupported[0].Metadata[key]
				if !ok {
					t.Fatalf("expected metadata key %q, got %+v", key, captured.Unsupported[0].Metadata)
				}
				if !httpMetadataValueEqual(wantVal, actVal) {
			...
			if name == "generated_by_default_identity_with_options" {
				opts, ok := captured.Unsupported[0].Metadata["identity_options"].(map[string]any)
				if !ok {
					t.Fatalf("expected identity_options to be map[string]any, got %T", captured.Unsupported[0].Metadata["identity_options"])
				}
				if !httpMetadataValueEqual(int32(10), opts["start"]) {
					t.Fatalf("expected identity_options.start=10, got %v", opts["start"])
				}
				if !httpMetadataValueEqual(int32(5), opts["increment"]) {
					t.Fatalf("expected identity_options.increment=5, got %v", opts["increment"])
				}
				if !httpMetadataValueEqual(true, opts["cycle"]) {
