...
| query_access.go | Implements the `query-access analyze` subcommand with SQL input loading, dialect-aware connection flags, mode/dialect/schema flags, JSON output, admission-based exit codes, identity-routed online analysis through the opaque unified SDK session, and bounded connection-failure translation for generic constructor/capability sentinels |
