...
| query_access.go | Implements the `query-access analyze` subcommand with SQL input loading, mode/dialect/schema flags, JSON output, admission-based exit codes, identity-routed online analysis through the opaque unified SDK session, and expected-dialect-based translation back to historical bounded constructor errors |
| query_access_test.go | Verifies query-access command JSON output, exit codes, field names, mode handling, unified online entry wiring with an empty analysis dialect, historical bounded constructor/capability failure presentation, close ownership, and no-audit-field-leakage |
...
| query_access_postgresql_online_recording_test.go | Recording-driver proof that the CLI PostgreSQL unified online path delegates COUNT(1) analysis without sending user SQL, EXPLAIN, or prepare operations, and keeps cancellation/closed-session failures bounded after probing the intended session |
| query_access_e2e_mixed_literal_test.go | Docker-backed in-process adapter matrix plus a separately built real-binary smoke across MySQL 5.7/8.0/8.4 and TiDB 8.5, including admitted mixed/literal shapes, offline regression, exact requirements, and marker no-leak assertions |
