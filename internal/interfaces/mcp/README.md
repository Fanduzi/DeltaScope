...
- `get_capabilities` is MCP-client-facing and summarizes transport, official tool names, audit modes, dialect support, top-level and connection inputs (including `connection.database` and `connection.connect_timeout`), audit result fields, context fields (`mode`, `dialect`, `dialect_source`, `schema`, `schema_source`, `metadata_source`, `note`, `unproven`), metadata features, and the stable structured error codes the server advertises (`bad_request`, `connection_invalid`, `connection_failed`, `config_invalid`).
