...
func TestAuditSQLToolPostgreSQLAlterTableGeneratedFollowUpBoundariesReturnUnsupported(t *testing.T) {
	server := NewServer(Config{Version: "test-version"})
	session, err := connectClientSession(context.Background(), server)
	if err != nil {
		t.Fatalf("connect session: %v", err)
...
// TestAuditSQLToolPostgreSQLGeneratedIdentityUnsupportedMetadata verifies that the
// MCP surface correctly reports unsupported errors for generated/identity columns.
//
// NOTE: The MCP surface only returns an error code and message for unsupported
// statements — it does NOT expose UnsupportedDetail.Metadata in the structured
// content. This test documents that limitation and asserts the existing contract
// (IsError=true, code non-empty, message contains "unsupported").
func TestAuditSQLToolPostgreSQLGeneratedIdentityUnsupportedMetadata(t *testing.T) {
	server := NewServer(Config{Version: "test-version"})
	session, err := connectClientSession(context.Background(), server)
	if err != nil {
		t.Fatalf("connect session: %v", err)
