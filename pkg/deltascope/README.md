...
- `DefaultVersion` is now `v0.16.2`, which matches the unified DeltaScope product surfaces and the current release baseline for source builds.
- The public package surface stays stable in `v0.16.2`; this hotfix keeps the exported `pkg/deltascope` contract additive while repairing the release-validation smoke configuration used for the converged Linux amd64 PostgreSQL archive path.
