...
- `DefaultVersion` is now `v0.13.1`, which matches the source-build baseline used by the CLI, HTTP server, and official MCP server release artifacts.
- The public package surface stays stable in `v0.13.1`; this patch release fixes the landing page JavaScript syntax without changing the exported `pkg/deltascope` contract.
