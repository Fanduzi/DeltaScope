...
- `DefaultVersion` is now `v0.11.1`, which matches the source-build baseline used by the CLI, HTTP server, and official MCP server release artifacts.
- The public package surface stays stable in `v0.11.1`; the release expands product entrypoints and CI/CD observability, not the exported `pkg/deltascope` contract.
