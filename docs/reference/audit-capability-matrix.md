...
| Offline trust context visibility | covered | CLI output formats (json, markdown, quiet) report audit context: markdown includes `## Audit Context` section with trust note and, when offline, `existence not checked (no database connection)` on Audit Context and Action Summary; JSON includes `context` with offline `note` / `unproven`; quiet includes a `[context]` line carrying the same caveat. The `github-actions` and `sarif` formats emit findings only and do not include context metadata. |
