...
| Rule ID | Check Description | Offline | Metadata | Default Level |
|---------|-------------------|:-------:|:--------:|---------------|
| `ddl.rename_table.notice` | `RENAME TABLE` renames a table | ✓ | ✗ | notice |
| `ddl.create_index.notice` | Standalone `CREATE INDEX` creates a new index | ✓ | ✗ | notice |
| `ddl.drop_index.notice` | Standalone `DROP INDEX` removes an index | ✓ | ✗ | notice |
| `ddl.alter_database.notice` | `ALTER DATABASE` changes database-level settings | ✓ | ✗ | notice |
| `ddl.create_procedure.notice` | `CREATE PROCEDURE` defines a stored procedure | ✓ | ✗ | notice |
| `ddl.drop_procedure.notice` | `DROP PROCEDURE` removes a stored procedure | ✓ | ✗ | notice |
| `ddl.create_user.notice` | `CREATE USER` creates a database user | ✓ | ✗ | notice |
| `ddl.alter_user.notice` | `ALTER USER` modifies a database user | ✓ | ✗ | notice |
| `ddl.drop_user.notice` | `DROP USER` removes a database user | ✓ | ✗ | notice |
| `ddl.create_role.notice` | `CREATE ROLE` creates a new role | ✓ | ✗ | notice |
| `ddl.drop_role.notice` | `DROP ROLE` removes a role | ✓ | ✗ | notice |
| `ddl.grant.notice` | `GRANT` assigns privileges | ✓ | ✗ | notice |
| `ddl.revoke.notice` | `REVOKE` removes privileges | ✓ | ✗ | notice |
| `ddl.drop_resource_group.notice` | `DROP RESOURCE GROUP` removes a resource group (MySQL-only) | ✓ | ✗ | notice |

### ALTER TABLE Action Notice Rules

| Rule ID | Check Description | Offline | Metadata | Default Level |
|---------|-------------------|:-------:|:--------:|---------------|
| `ddl.alter.add_column.notice` | `ALTER TABLE ... ADD COLUMN` adds a column | ✓ | ✗ | notice |
| `ddl.alter.add_constraint.notice` | `ALTER TABLE ... ADD CONSTRAINT` adds a constraint | ✓ | ✗ | notice |
| `ddl.alter.drop_column.notice` | `ALTER TABLE ... DROP COLUMN` removes a column | ✓ | ✗ | notice |
| `ddl.alter.modify_column.notice` | `ALTER TABLE ... MODIFY COLUMN` changes column definition | ✓ | ✗ | notice |
| `ddl.alter.drop_index.notice` | `ALTER TABLE ... DROP INDEX` removes an index | ✓ | ✗ | notice |
| `ddl.alter.drop_foreign_key.notice` | `ALTER TABLE ... DROP FOREIGN KEY` removes a foreign key | ✓ | ✗ | notice |

### TiDB-Specific Rules

| Rule ID | Check Description | Offline | Metadata | Default Level |
|---------|-------------------|:-------:|:--------:|---------------|
| `ddl.create_placement_policy.notice` | `CREATE PLACEMENT POLICY` defines a placement policy | ✓ | ✗ | notice |
| `ddl.alter_placement_policy.notice` | `ALTER PLACEMENT POLICY` modifies a placement policy | ✓ | ✗ | notice |
| `ddl.drop_placement_policy.notice` | `DROP PLACEMENT POLICY` removes a placement policy | ✓ | ✗ | notice |
| `ddl.create_sequence.notice` | `CREATE SEQUENCE` defines a sequence | ✓ | ✗ | notice |
| `ddl.alter_sequence.notice` | `ALTER SEQUENCE` modifies a sequence | ✓ | ✗ | notice |
| `ddl.drop_sequence.notice` | `DROP SEQUENCE` removes a sequence | ✓ | ✗ | notice |
| `ddl.tidb.alter_table.placement_policy.notice` | `ALTER TABLE ... PLACEMENT POLICY` attaches a placement policy to a table | ✓ | ✗ | notice |

> **Note:** These rules are MySQL/TiDB-specific and are automatically skipped when auditing PostgreSQL SQL. They are offline rules and do not require a database connection. These are informational notice rules only — they do not block, warn, or perform live validation. `ddl.drop_resource_group.notice` is MySQL-only. TiDB placement policy and sequence rules are automatically skipped for MySQL. This is not full MySQL/TiDB DDL support — triggers, events, functions, ALTER VIEW, ALTER PROCEDURE, tablespace, and parser-error forms remain deferred.
...
| `ddl.pg.alter.add_inherit.notice` | `ALTER TABLE ... INHERIT` adds a parent table — informational notice (v0.150.0) | ✓ | ✗ | notice |
...
| ALTER TABLE coverage rules (v0.51.0 / v0.52.0 / v0.54.0 / v0.56.0 / v0.130.0 / v0.140.0 / v0.150.0 / v0.170.0) | `ddl.pg.alter.drop_column.advisory`, `ddl.pg.alter.validate_constraint.advisory`, `ddl.pg.alter.add_column.nullable.notice`, `ddl.pg.alter.set_schema.advisory`, `ddl.pg.alter.owner.advisory`, `ddl.pg.alter.enable_trigger.notice`, `ddl.pg.alter.disable_trigger.warn`, `ddl.pg.alter.attach_partition.advisory`, `ddl.pg.alter.detach_partition.warn`, `ddl.pg.alter.replica_identity_full.warn`, `ddl.pg.alter.replica_identity_nothing.warn`, `ddl.pg.alter.replica_identity_using_index.notice`, `ddl.pg.alter.set_logged.notice`, `ddl.pg.alter.set_unlogged.notice`, `ddl.pg.alter.set_tablespace.notice`, `ddl.pg.alter.set_access_method.warn`, `ddl.pg.alter.enable_replica_trigger.notice`, `ddl.pg.alter.enable_always_trigger.notice`, `ddl.pg.alter.enable_rule.notice`, `ddl.pg.alter.disable_rule.warn`, `ddl.pg.alter.enable_replica_rule.notice`, `ddl.pg.alter.enable_always_rule.notice`, `ddl.pg.alter.set_reloptions.warn`, `ddl.pg.alter.reset_reloptions.notice`, `ddl.pg.alter.set_column_statistics.notice`, `ddl.pg.alter.set_column_options.notice`, `ddl.pg.alter.reset_column_options.notice`, `ddl.pg.alter.set_column_storage.notice`, `ddl.pg.alter.set_column_compression.notice`, `ddl.pg.alter.cluster_on.notice`, `ddl.pg.alter.set_without_cluster.notice`, `ddl.pg.alter.detach_partition_finalize.notice`, `ddl.pg.alter.add_inherit.notice`, `ddl.pg.alter.drop_inherit.notice`, `ddl.pg.alter.add_of_type.notice`, `ddl.pg.alter.drop_of_type.notice`, `ddl.pg.alter.set_expression.notice`, `ddl.pg.alter.add_identity.notice`, `ddl.pg.alter.add_exclusion_constraint.notice`, `ddl.pg.alter.move_all_tablespace.notice` — offline PostgreSQL-specific rules covering ALTER TABLE safety patterns. Trigger-scope forms (`ALL/USER`) normalized since v0.54.0. `REPLICA IDENTITY DEFAULT` normalized and intentionally silent. Offline only — DeltaScope does not verify `REPLICA IDENTITY USING INDEX` index validity. DeltaScope does not verify whether the target table is currently logged or unlogged. |
...
| Capability | Status | Notes |
|------------|--------|-------|
| PostgreSQL syntax heuristic notice | covered | When auditing on MySQL/TiDB path, detects common PG-specific syntax tokens (`ON CONFLICT`, `::`, `ALTER COLUMN TYPE USING`, `GENERATED AS IDENTITY`) and emits `dialect.postgresql.syntax.detected.notice` as a global advisory finding. `RETURNING` is no longer in this list; it is valid TiDB syntax. DeltaScope does not auto-switch dialect. |
| MySQL DML RETURNING unsupported notice | covered | On the MySQL dialect, emits `dialect.mysql.returning.unsupported.notice` as a global advisory finding when a parsed DML statement carries a `RETURNING` clause (the TiDB parser recognizes `INSERT`, `UPDATE`, and single-table `DELETE` `RETURNING`). MySQL Server does not support DML `RETURNING`; re-run with `--dialect tidb` if the SQL targets TiDB. TiDB dialect accepts `RETURNING` and does not fire this notice. |
...
| Offline trust context visibility | covered | CLI json/markdown/quiet, HTTP `POST /v1/audit`, and MCP `audit_sql` report audit context. Offline `context` includes `note` (`existence not checked (no database connection)`) and `unproven` (`column_exists`, `table_exists`) on CLI JSON, HTTP JSON, and MCP `structuredContent`. CLI markdown puts the same line on Action Summary and Audit Context; quiet puts it on `[context]`; MCP `content[0].text` adds that one line to the compact summary and does not dump structured context. Metadata-aware results omit `note` / `unproven`. `pkg/deltascope.Result` has no `context`. The `github-actions` and `sarif` formats emit findings only and do not include context metadata. |
| Rule summary / skipped rules visibility | covered | CLI output formats (json, markdown, quiet) report Loaded, applicable, and skipped rule counts. Loaded is not the Rule Catalog size and not the Default Policy key count. FK-forbid naming suppression is named by `config status` as `fk_forbid`. The `github-actions` and `sarif` formats emit findings only and do not include rule summary metadata. |
...
| Parser-error unsupported contract | covered | When the selected dialect parser cannot parse a tracked DDL statement, all public surfaces (SDK, CLI, HTTP, MCP) return a diagnostic stating that no audit was performed and no findings were inferred from unparsed SQL. This is not a fallback parser — DeltaScope does not extract facts from or produce findings based on parser-error SQL. No parser support, fallback parsing, or new SQL audit rules are added. The parser-error count is not reduced. |
| Unsupported diagnostics evidence (v0.230.0) | covered | Parser-error and unsupported statement outcomes expose structured diagnostic evidence (`classification`, `reason`, `action_hint`, `audited`, `dialect`, `guidance_code`, `evidence_ref`) across all public surfaces (SDK, CLI JSON/text, HTTP, MCP). `guidance_code` and `evidence_ref` are optional (v0.260.0+) and appear only when the diagnostic matches a known unsupported boundary. Diagnostics do not contain raw SQL text, parser `near ...` fragments, routine bodies, or any other forbidden payload. This is not parser support, not a fallback parser, and not new SQL audit rules. Parser-error counts are not reduced. The census is unchanged. |
| Parser upgrade candidate evidence (v0.250.0) | classified | Remaining parser-error DDL cases (29 total: MySQL 15, TiDB 9, PostgreSQL 5) are classified by feasibility bucket. `parser_upgrade_candidate` (10 cases: MySQL 5, PostgreSQL 5) identifies forms that would become parseable after a parser/library upgrade. Other buckets: `bounded_fallback_candidate` (4), `product_unsupported_or_inapplicable` (6), `unsafe_fallback_defer` (9), `needs_research` (0). `parser_upgrade_candidate` is not current parser support. DeltaScope does not infer findings from failed parse text. No fallback parser is used. Users should treat parser-error statements as not audited and review them manually. CLI output shape is unchanged. `parser_upgrade_candidate` is a documented classification, not a new CLI field. No parser support is added, no new SQL audit rules are created, and parser-error counts are not reduced. |
...
| DDL coverage catalog | covered | A verified representative catalog of DDL forms classified by audit status per dialect. See [ddl-coverage.md](ddl-coverage.md). Machine-readable JSON at [`ddl-coverage-catalog.json`](ddl-coverage-catalog.json). This is not a full DDL support claim, not a dialect parity declaration, and not a vendor grammar completeness claim. |
