...
| Unsupported diagnostics evidence (v0.230.0) | covered | Parser-error and unsupported statement outcomes expose structured diagnostic evidence (`classification`, `reason`, `action_hint`, `audited`, `dialect`) across all public surfaces (SDK, CLI JSON/text, HTTP, MCP). Diagnostics do not contain raw SQL text, parser `near ...` fragments, routine bodies, or any other forbidden payload. This is not parser support, not a fallback parser, and not new SQL audit rules. Parser-error counts are not reduced. The census is unchanged. |
