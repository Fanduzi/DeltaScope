...
| `ddl.pg.alter.add_inherit.notice` | `ALTER TABLE ... INHERIT` 添加父表——信息性提示 (v0.150.0) | ✓ | ✗ | notice |
| `ddl.pg.alter.drop_inherit.notice` | `ALTER TABLE ... NO INHERIT` 移除父表——信息性提示 (v0.150.0) | ✓ | ✗ | notice |
| `ddl.pg.alter.add_of_type.notice` | `ALTER TABLE ... OF type_name` 将表绑定到类型表——信息性提示 (v0.150.0) | ✓ | ✗ | notice |
...
| 离线信任上下文可见性 | 已覆盖 | CLI 输出格式（json、markdown、quiet）报告审计上下文：markdown 包含 `## Audit Context` 区段和信任提示，离线时在 Audit Context 与 Action Summary 标明 `existence not checked (no database connection)`；JSON 的 `context` 含离线 `note` / `unproven`；quiet 的 `[context]` 行带同一说明。`github-actions` 和 `sarif` 格式仅输出告警结果，不包含上下文元数据。 |
...
| Parser upgrade candidate evidence（v0.250.0） | 已分类 | 剩余 parser-error DDL 用例（共 29 条：MySQL 15、TiDB 9、PostgreSQL 5）已按可行性 bucket 分类。`parser_upgrade_candidate`（10 条：MySQL 5、PostgreSQL 5）标识了在 parser/库升级后可变为可解析的形态。其余 bucket：`bounded_fallback_candidate`（4 条）、`product_unsupported_or_inapplicable`（6 条）、`unsafe_fallback_defer`（9 条）、`needs_research`（0 条）。`parser_upgrade_candidate` 不是当前 parser 支持。DeltaScope 不从解析失败文本推断 findings。不启用 fallback parser。用户应将 parser-error 语句视为未审计并手动审查。CLI 输出结构不变。`parser_upgrade_candidate` 是文档化分类，不是新的 CLI 字段。不增加 parser 支持、不新增 SQL 审计规则、parser-error 数量不会减少。 |
...
| DDL 覆盖目录 | 已覆盖 | 按方言和审计状态分类的已验证代表性 DDL 形态目录。见 [ddl-coverage.zh-CN.md](ddl-coverage.zh-CN.md)。机器可读 JSON 位于 [`ddl-coverage-catalog.json`](ddl-coverage-catalog.json)。这不是完整 DDL 支持声明、不是方言对等声明、不是数据库厂商语法完整性声明。 |
