...
Exit codes: `0` = admissible, `1` = rejected, `2` = indeterminate, `3` = usage error. The CLI always emits the fixed Query Access JSON document; audit-only `--format` and `--fail-on` flags are unsupported and fail with exit 3 without analysis output.
