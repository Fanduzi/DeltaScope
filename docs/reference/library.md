...
| `Dialect` | No | `DialectMySQL`, `DialectTiDB`, or `DialectPostgreSQL`. Defaults to `DialectMySQL` when the zero value (`""`) is provided. PostgreSQL requests require a PG-capable build. Starting with the `v0.17.0` public release line, the supported macOS and Linux main archives are PG-capable, so PostgreSQL offline API usage follows the same main product path as MySQL and TiDB. |
