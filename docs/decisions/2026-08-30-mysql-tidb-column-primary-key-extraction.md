...
column extractor already preserves the required attribute facts. Adding the
missing AST-to-spec mapping at the shared CREATE TABLE extraction point keeps
MySQL and TiDB behavior aligned without changing policy, rule logic, or the
public report shape.
...
- A single inline primary key satisfies `ddl.table.primary_key.require` and
  its extracted column attributes remain available to the existing semantic
  rules.
