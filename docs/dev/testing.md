...
remains the offline semantic owner.
...
- The pull-request lint workflow runs `python3 scripts/test_pr_workflow_contract.py` and `make sql-corpus-gates` without Docker. Separate unguarded jobs on the same `pull_request` / `push` to `main` workflow run `go test ./...` and `make pg-unit-test-gates`. Full Docker metadata e2e stays on release or manual smoke workflows, not on every pull request.
