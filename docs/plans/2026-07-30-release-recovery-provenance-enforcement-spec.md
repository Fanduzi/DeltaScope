...
3. After the post-tag candidate gate succeeds, `preflight` resolves the
...
4. `publish-homebrew-cask` and `publish-mcp-launcher-package` check out
...
5. Every job capable of mutating an external release surface remains directly
   or transitively dependent on successful `preflight`.
6. The provenance check runs for dry-run and non-dry-run recovery. Dry-run
   avoids publisher mutation; it does not weaken admission of the requested
   tag.
7. There is no workflow input, environment switch, or historical-version
   allowlist that bypasses provenance verification.
8. The `preflight` job declares explicit job-level
