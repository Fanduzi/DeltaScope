...
| `query_access_online_session_postgresql_tag_test.go`: `TestOnlineQueryAccessSession_PostgreSQLMatchesLegacyAPI` rows other than `COUNT(1)` | duplicate tagged PG17 semantic equivalence | S3 | C2 retains one `COUNT(1)` routing row | Deleted by Issue #11 after focused tagged SDK green evidence |
| `query_access_online_session_postgresql_integration_test.go`: `TestUnifiedSession_MatchesLegacyPG17` rows other than `COUNT(1)` | duplicate live PG17 semantic equivalence | U2 | C2 retains one `COUNT(1)` routing row | Deleted by Issue #11 after focused PostgreSQL Docker green evidence |
