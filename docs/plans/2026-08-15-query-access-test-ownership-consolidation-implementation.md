...
| `query_access_online_session_postgresql_tag_test.go`: `TestOnlineQueryAccessSession_PostgreSQLMatchesLegacyAPI` table rows `SELECT COUNT(*) FROM app.users`, `SELECT COUNT(NULL) FROM app.orders`, `SELECT COUNT(1) FILTER (WHERE true) FROM app.orders`, `SELECT COUNT(1)`, `SELECT COUNT(1) FROM app.orders JOIN app.users ON true`, `SELECT COUNT(1) FROM app.remote_orders`, `SELECT count(amount), sum(amount), avg(amount), min(amount), max(amount) FROM app.orders`, `SELECT row_number() OVER (PARTITION BY user_id ORDER BY amount) FROM app.orders`, and `SELECT u.id FROM app.users u JOIN app.orders o ON u.id = o.user_id` | duplicate tagged PG17 semantic equivalence | S3 | C2 retains one `COUNT(1)` routing row | Deleted by Issue #11 after focused tagged SDK green evidence |
...
| `query_access_e2e_mixed_literal_test.go` CLI profile table rows | repeated MySQL/TiDB product/profile/shape results | U1 | T1 real-route smoke per family | Blocked: retain for Issue #12 CLI transport scope |
...
| `query_access_postgresql_online_recording_test.go`: `TestCLIOnlinePG17_CountIntegerOne_Recording` fixed-probe assertions | duplicated adapter probe sequence | U3 | T1 | Blocked: retain for Issue #12 CLI transport scope |
