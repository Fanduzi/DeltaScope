...
A bounded scalar function subset (LOWER, UPPER, LENGTH, CHAR_LENGTH, ABS, CEIL, FLOOR, COALESCE, NULLIF, IFNULL) is promoted for online Query Access when the connected server identity matches a supported series (MySQL 5.7.x, MySQL 8.0.x, MySQL 8.4.x, TiDB 8.5.x, PostgreSQL 17.x). The subset is intentionally narrow. Every operand of every listed scalar function must be a direct physical base-table column; nested expressions, literals, and function calls are rejected.
...
- TLS mode is declared per connection: `disabled` (default) or `enabled`. When enabled, certificate-chain and hostname verification cannot be disabled. `tls_ca_file` is optional; when absent, system trust roots are used. `tls_mode: disabled` intentionally uses no TLS. No TLS configuration is accepted on the HTTP request body.
- Bounded scalar function subset promoted for online Query Access: LOWER, UPPER, LENGTH, CHAR_LENGTH, ABS, CEIL, FLOOR, COALESCE, NULLIF, IFNULL. Every operand must be a direct physical base-table column. IFNULL is MySQL/TiDB-only (N/A on PostgreSQL).
- Online sessions derive capability from connected-server identity: MySQL 5.7.x, MySQL 8.0.x, MySQL 8.4.x, TiDB 8.5.x, PostgreSQL 17.x. No caller-supplied manifest or profile override is accepted.
...
| ABS, CEIL, FLOOR | ✓ | ✓ | ✓ | ✓ | ✓ |
| COALESCE, NULLIF | ✓ (direct col) | ✓ (direct col) | ✓ (direct col) | ✓ (direct col) | ✓ (direct col) |
| IFNULL | ✓ (direct col) | ✓ (direct col) | ✓ (direct col) | ✓ (direct col) | N/A |

Every operand of every listed scalar function must be a direct physical base-table column. Nested expressions, literals, and function calls inside these operators are not supported. IFNULL is a MySQL/TiDB builtin with no PostgreSQL equivalent.
