# DeltaScope v0.510.0 发行说明

状态：官方 GitHub Actions 运行 33302045413 通过 provenance 后，在 `TestAuditCommandLoadsTLSCAFile` 失败，且尚未产出任何资产。这不是一次成功的已发布 GitHub Release。v0.510.1 再发布这份源码工作并附带 Linux 测试契约修正；见 `docs/releases/release-notes-v0.510.1.zh-CN.md`。

## 概要 - 发布通道解耦、CLI JSON 压缩、解析 review 下限与连接拒绝分类

v0.510.0 候选包含 v0.500.0 之后落地的 #54–#57 源码工作。在 provenance 与平台构建成功的前提下，即使 Homebrew cask 发布或安装验证失败，tag 触发的工作流也会发布 `@fanduzi/deltascope-mcp`；本次记录的运行在更早的 release tests 阶段停止。默认 CLI JSON 把方言上大量的 skip 列表压缩为 `{reason, count}` 聚合；`--include-skipped-rules` 恢复逐规则列表。混合迁移在存在未审计 `parser_error` 诊断时，即使 findings 只有 notice，也不会再给出 `pass`，而是在 SDK、CLI、HTTP、MCP 上落到 `review`。元数据感知 CLI 的 TCP 拒绝是有界的 `connection refused`，退出码 3。
...
- 不是补发 `@fanduzi/deltascope-mcp@0.500.0`。
...
- `docs/decisions/2026-08-30-release-channel-npm-homebrew-parity.md`（本版本；#54）
- `docs/decisions/2026-08-30-cli-json-skipped-rule-compaction.md`（本版本；#55）
- `docs/decisions/2026-08-30-partial-parser-error-verdict-review-floor.md`（本版本；#56）
- `docs/decisions/2026-08-30-cli-connection-error-categories.md`（本版本；#57）
